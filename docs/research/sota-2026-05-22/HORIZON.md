...
| **B** | Build `radiofrequencia` MCP server + CLI (`tools/radiofrequencia-mcp/`, `tools/radiofrequencia-cli/`) | `feat/radiofrequencia-mcp-cli` |
| **C** | Write ADR-104: radiofrequencia MCP/CLI distribution decision record | (same branch as B) |

---

## Milestones

### M1 — Scaffold `tools/radiofrequencia-mcp/` + `tools/radiofrequencia-cli/`
**Target:** +1h (by ~21:00 ET)
**Status:** `COMPLETE` — merged as PR #705 (squash commit `5a6c585aa`)
**Branch:** `feat/radiofrequencia-mcp-cli-pr` (deleted after merge)

Deliverables:
- `tools/radiofrequencia-mcp/package.json` — `@ruv/radiofrequencia-mcp`, TypeScript, `@modelcontextprotocol/sdk`
- `tools/radiofrequencia-mcp/src/index.ts` — minimal MCP server with 5 tool stubs
- `tools/radiofrequencia-mcp/src/tools/` — one file per tool
- `tools/radiofrequencia-cli/package.json` — `@ruv/radiofrequencia-cli` + `radiofrequencia` bin
- `tools/radiofrequencia-cli/src/index.ts` — 4-verb CLI stub via yargs/commander
- `tsconfig.json` for both packages
- Shared `tools/radiofrequencia-shared/` for HTTP client + types

Completion criteria: `npm run build` succeeds in both packages, MCP server can be registered with `claude mcp add`.

---

### M2 — Wire `radiofrequencia_pose_infer` + `radiofrequencia_count_infer`
...
Completion criteria met: `radiofrequencia_pose_infer` returns finite keypoint array (17 COCO keypoints, confidence-gated); `radiofrequencia_count_infer` returns `{count, confidence, count_p95_low, count_p95_high}`.

---

### M3 — Wire `radiofrequencia_csi_latest` + `radiofrequencia_registry_list`
...
### M4 — Wire `radiofrequencia_train_count`
**Target:** +7h (by ~03:00 ET)
**Status:** `COMPLETE` — implemented in PR #705 + #708; `radiofrequencia_train_count` spawns detached cargo process, returns `{job_id, status:"queued"}` via UUID; log streamed to `~/.radiofrequencia/jobs/<id>.log` using fd-based detach (Windows-compatible).

Completion criteria met: returns `{job_id, status: "queued"}` within 200 ms (detached subprocess, no blocking).

---

### M5 — ADR-104: radiofrequencia MCP/CLI distribution
**Target:** +8h (by ~04:00 ET)
**Status:** `COMPLETE` — ADR-104 written and merged in PR #705 (Session 1)

Full ADR covering: problem, design (5 MCP tools + 5 CLI subcommands + library mapping), security (6-row threat table), packaging (npm `@ruv/radiofrequencia-mcp` + `@ruv/radiofrequencia-cli`), distribution, failure modes, acceptance gates.

Completion criteria: ADR file at `docs/adr/ADR-104-radiofrequencia-mcp-cli-distribution.md`, merged to main.
...
|------|----|-------------|--------|
| `tools/radiofrequencia-mcp/` scaffold (6 tools, TypeScript ESM, MCP SDK) | #705 | `5a6c585aa` | Shipped |
| `tools/radiofrequencia-cli/` scaffold (6 subcommands, Yargs) | #705 | `5a6c585aa` | Shipped |
| ADR-104 (radiofrequencia MCP/CLI distribution, 6-row threat table) | #705 | `5a6c585aa` | Shipped |
...
|------|--------|-----------|
| `radiofrequencia_csi_latest` with real running sensing-server (live E2E test) | sensing-server not running in CI; graceful WARN path tested instead | Run against `cognitum-v0` when fleet is available |
...
| `radiofrequencia_registry_list` live response (real edge registry) | graceful WARN path tested; no edge registry in local CI | Run against `cognitum-v0:9000/edge` |
...
npm run build
npm publish --access public   # publishes @ruv/radiofrequencia-mcp

cd ../radiofrequencia-cli
npm run build
npm publish --access public   # publishes @ruv/radiofrequencia-cli
```
...
- A fully wired MCP server (`@ruv/radiofrequencia-mcp`) with 6 tools, schema validation, fail-open pattern, 16 passing tests.
- A matching CLI (`@ruv/radiofrequencia-cli`) with 6 subcommands.
...
- `tools/radiofrequencia-mcp/` and `tools/radiofrequencia-cli/` scaffolded with TypeScript, MCP SDK, Yargs.
...
- `docs/adr/ADR-104-radiofrequencia-mcp-cli-distribution.md` written (full depth, 6-row threat table).
...
**Started:** Context resumed from prior session summary. Branch `feat/radiofrequencia-mcp-m3-m4` active from main at `6b3589684`.
**Accomplished:**
...
- **M4 formally closed:** `radiofrequencia_train_count` (spawns detached cargo process, UUID job_id, log via fd, <200ms) was implemented in the prior session; milestone retroactively marked COMPLETE.
- **M5 formally closed:** ADR-104 was merged in Session 1 (PR #705); milestone retroactively marked COMPLETE.
- **M6 formally closed:** 16 passing tests satisfy "npm test passes in tools/radiofrequencia-mcp/" criterion.
