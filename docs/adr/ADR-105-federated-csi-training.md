...
Adopt **MERIDIAN-FedAvg with byzantine-robust aggregation** as the radiofrequencia federated training protocol.
...
Per round (typical radiofrequencia 4-seed installation):
...
- **NEW: `radiofrequencia-fed` crate** — protocol implementation, ~500 lines Rust, library only (no daemon).
...
1. radiofrequencia personalisation becomes possible **without** violating R14/R3 privacy constraints.
...
1. Adds ~500 lines of new Rust code (the `radiofrequencia-fed` crate).
2. Krum is O(K²) in nodes — fine for K ≤ 50 (typical radiofrequencia installation), expensive for K > 1000 (not a target).
...
- **ADR-103 (cog-person-count):** the v0.0.2 retrained model from this loop's earlier work would be the first cog to use the federation protocol — once `radiofrequencia-fed` ships.
- **ADR-104 (radiofrequencia-mcp + radiofrequencia-cli):** federation status surfaces as MCP tools (`radiofrequencia_fed_status`, `radiofrequencia_fed_pause`) — out of scope for this ADR but in the natural MCP roadmap.

## Implementation plan

| Step | Owner | LOC | Notes |
|---|---|---:|---|
| 1. `radiofrequencia-fed` crate scaffold | TBD | 100 | Workspace member, no external deps initially |
...
Total ~500 lines + tests. A reasonable 2-week effort once `radiofrequencia-fed` is unblocked.
...
- **R8 / R5:** first cog to use the federation protocol once `radiofrequencia-fed` ships.
...
- Krum is byzantine-robust against `f < (K-2)/2` byzantine nodes. For K=4, that means 1 byzantine; for K=10, 4. radiofrequencia installations rarely have K>10 seeds, so the practical bound is ~4 byzantine.
