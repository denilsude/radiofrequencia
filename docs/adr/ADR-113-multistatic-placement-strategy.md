...
Adopt the **4-axis placement decision matrix** below as the binding radiofrequencia installation specification.
...
```
radiofrequencia_placement_recommend(
...
4. **MCP tool unblocks AI-agent-driven deployment** — empathic appliance integration partners can call `radiofrequencia_placement_recommend` programmatically.
...
- **ADR-104 (radiofrequencia-mcp + radiofrequencia-cli)**: `radiofrequencia_placement_recommend` becomes a new MCP tool.
...
| 2. MCP tool `radiofrequencia_placement_recommend` | 80 | TBD |
